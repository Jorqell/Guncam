...
DO
    LOCATE 1
    PRINT "Jorqell's Guncam 1.07  19.12.2020"
    ...
    IF d& THEN '             the device number cannot be zero!
        ...
        FOR b = 1 TO _LASTBUTTON(d&)
            IF _BUTTONCHANGE(b) = 1 THEN nro = b: dnro = d&: eteenp = 1: PRINT "Device"; dnro; "button"; nro; "selected.": PRINT "Starting test recording": _DISPLAY: _DELAY 1
        NEXT
        PRINT
    END IF
LOOP UNTIL eteenp = 1
...
PRINT
PRINT "+ and - change brightness "; expy
PRINT "/ and * change contrast "; cont
PRINT "WASD moves camera"
PRINT "JL aspect ratio "; aspect
PRINT "IK zoom"
PRINT "V notification sounds"; volume
...
IF r$ = "*" THEN cont = cont - 1
IF r$ = "w" THEN yoff = yoff - 4
IF r$ = "s" THEN yoff = yoff + 4
IF r$ = "a" THEN xoff = xoff - 4
IF r$ = "d" THEN xoff = xoff + 4
...
IF r$ = "k" AND zoom > 40 THEN zoom = zoom - 1
IF r$ = "j" THEN aspect = aspect - 1
IF r$ = "l" THEN aspect = aspect + 1
...
IF scxa < 0 THEN scxa = 0: xoff = xoff + 10
IF scxb > _DESKTOPWIDTH THEN scxb = _DESKTOPWIDTH: xoff = xoff - 10
IF scya < 0 THEN scya = 0: yoff = yoff + 10
IF scyb > _DESKTOPHEIGHT THEN scyb = _DESKTOPHEIGHT: yoff = yoff - 10
...
FOR u% = 0 TO 400
    ...
    NEXT
NEXT
IF vakain = 1 THEN GOSUB kuvanvakain ELSE siirros = 0
...
IF autob = 1 THEN
    FOR u% = 0 TO 400 'check
        ...
        NEXT
    NEXT
    zurvo = INT((zurvo / (400 * 300)) - 128) / 4 - 10 'extra
END IF
...
FOR kek = 1 TO bluramount 'blur
    FOR u% = 1 TO 399
        FOR o% = 1 TO 299
            ...
            REM             SHIT% = (taulukkoa(u%, o%) + taulukkoa(u% + 1, o%) + taulukkoa(u% - 1, o%) + taulukkoa(u%, o% + 1) + taulukkoa(u%, o% - 1)) / 5
            taulukkob(u%, o%) = SHIT%
        NEXT
    NEXT
NEXT
...
FOR u% = 0 TO 400 'check
    FOR o% = 0 TO 300
        taulukkoa(u%, o%) = taulukkob(u%, o%) - zurvo
...
FOR kekkuli = 0 TO 200
    arvo = taulukkoa(150, 300 - kekkuli)
    IF arvo > 50 THEN vaka = vaka + 1
    IF arvo < 50 THEN vakb = vakb + 1
NEXT
IF vakb > siirros + 20 THEN siirrosm = siirrosm + (vakb / 20) * (0.5 + RND * 0.5)
IF vakb < siirros - 10 THEN siirrosm = siirrosm - 5 - (RND * 5)
